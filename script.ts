...
function DrawGayTriangle(program, triangleVertices) {
	const ObjectVerticesCpuBuffer = new Float32Array(triangleVertices);
...
`;
	...
	gl.vertexAttribPointer(
		vertexPositionLocation, // location
		2, // size (x, y)
		gl.FLOAT,
		false,
		stride,
	...
	gl.vertexAttribPointer(
		colorAttribLocation, // Attribute location for color
		...
		2 * Float32Array.BYTES_PER_ELEMENT, // Offset to the color data
	);
...
triangles.forEach((triangle) => {
	DrawGayTriangle(program, triangle);
});
